...
GAMMA = 0.99
CLIP_EPSILON = 0.2
BATCH_EPISODES = 64      # Nombre d'épisodes collectés avant mise à jour
NUM_EPOCHS = 10          # Nombre d'epochs d'optimisation sur le batch
MINIBATCH_SIZE = 64      # Taille du mini-batch lors de l'optimisation
...
# -----------------------------------------------------------
# Réseau Actor-Critic avec deux entrées (hand et adv)
# -----------------------------------------------------------
class ActorCriticNetwork(nn.Module):
    def __init__(self, hidden_dim, action_dim):
        super(ActorCriticNetwork, self).__init__()
        # Traitement de la main de l'agent (26 cartes, 5 dimensions chacune)
        self.hand_fc = nn.Linear(5, hidden_dim)
        # Traitement de la carte de l'adversaire (5 dimensions)
        self.adv_fc = nn.Linear(5, hidden_dim)
        # Combinaison des représentations :
        # La main donnera 26 * hidden_dim, et la carte de l'adversaire donne hidden_dim.
    ...
    def forward(self, hand, adv):
        ...
        x = torch.cat([hand_repr, adv_repr], dim=1)  # (batch, 26*hidden_dim + hidden_dim)
        x = F.relu(self.fc1(x))
        x = F.relu(self.fc2(x))
...
class PPOAgent:
    def __init__(self, hidden_dim=128, lr=LR):
        # La dimension d'entrée n'est plus fixée ici car nous recevons deux entrées
        # La main de l'agent a forme (26, 5) et la carte de l'adversaire (5,)
        # La sortie reste de dimension NB_CARTES_AGENT (26 actions possibles)
    ...
    def select_action(self, obs, valid_mask):
        """
        Pour une observation (dictionnaire avec 'hand' et 'adv') et un masque d'action,
        retourne l'action choisie, la log-probabilité associée et la valeur.
        """
        ...
        mask_tensor = torch.from_numpy(valid_mask).float()  # valid_mask de taille (26,)
    ...
    def update(self, batch):
        ...
        Le batch est un dictionnaire contenant les clés suivantes :
          'hand' : liste d'observations de la main (chaque élément de forme (26,5))
          'adv'  : liste d'observations de la carte adversaire (chaque élément de forme (5,))
          'actions', 'log_probs', 'returns', 'advantages'
        """
        hand_obs = torch.tensor(np.array(batch['hand']), dtype=torch.float32)  # (N, 26, 5)
        adv_obs = torch.tensor(np.array(batch['adv']), dtype=torch.float32)    # (N, 5)
        ...
        total_samples = hand_obs.shape[0]
        ...
        for epoch in range(NUM_EPOCHS):
            ...
            for start in range(0, total_samples, MINIBATCH_SIZE):
                ...
                mb_hand = hand_obs[mb_idx]
                mb_adv = adv_obs[mb_idx]
                mb_actions = actions[mb_idx]
                mb_old_log_probs = old_log_probs[mb_idx]
                mb_returns = returns[mb_idx]
                mb_advantages = advantages[mb_idx]
                
                logits, values = self.model(mb_hand, mb_adv)
    ...
    def collect_trajectories(self, env, batch_episodes=BATCH_EPISODES):
        ...
        Le batch est structuré en stockant séparément 'hand' et 'adv'.
        """
        batch = {
            'hand': [],
            'adv': [],
        ...
        for ep in range(batch_episodes):
            obs = env.reset()  # obs est un dictionnaire {'hand': (26,5), 'adv': (5,)}
            done = False
            ep_hand = []
            ep_adv = []
            ep_actions = []
            ...
            ep_values = []
            while not done:
                valid_mask = env.get_valid_action_mask()  # taille (26,)
            ...
            batch['hand'].extend(ep_hand)
            batch['adv'].extend(ep_adv)
