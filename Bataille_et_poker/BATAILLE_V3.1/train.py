...
# Environnement du jeu de cartes mis à jour avec masque d'actions
class CardGameEnv(gym.Env):
    """
    Environnement du jeu de cartes avec main limitée et masque d'actions :
      ...
      - Un masque d'actions "action_mask" est ajouté dans l'observation pour indiquer les actions légales.
      ...
      - Si l'action est illégale (la carte a déjà été jouée), le reward est -1 et l'état ne change pas,
        l'agent est invité à réessayer.
    ...
    def step(self, action):
        ...
        if self.hand[action] == 0:
            # Action illégale : pas de changement d'état
            reward = -1
            ...
            # On ne modifie pas la main ni la carte sur la table
            obs = {
                "table_card": np.array(self.table_card, dtype=np.int32),
                "hand": self.hand.copy(),
                "action_mask": self.hand.copy()
            ...
            # Action légale : on met à jour la main et on évalue le coup
            self.hand[action] = 0
        ...
        # Seul le cas d'action légale conduit à un changement de carte sur la table.
        # Pour une action illégale, la table reste inchangée.
